...
function Products() {
  ...
  if (showForm) return <section><div className="admin-heading"><div><div className="eyebrow">Catalogue</div><h1 className="display">{editing ? 'Edit product' : 'Add product'}</h1></div></div><ProductForm product={editing} onDone={() => { setShowForm(false); setEditing(undefined); }} /></section>;
...
export default function AdminApp() {
  ...
  return <div className="admin-shell"><aside className="admin-sidebar"><div className="admin-brand"><span className="eyebrow">Sole / House</span><strong>Seller workspace</strong></div><nav className="admin-nav" aria-label="Seller navigation">{[['overview', 'Overview'], ['products', 'Products'], ['orders', 'Orders'], ['settings', 'Settings']].map(([value, text]) => <button className={tab === value ? 'active' : ''} key={value} onClick={() => setTab(value as typeof tab)}>{text}</button>)}</nav><a className="admin-storefront-link" href="/" target="_blank" rel="noreferrer">View storefront ↗</a><button className="admin-signout" onClick={signOut}>Sign out</button></aside><main className="admin-main">{tab === 'overview' && <Overview onOrders={() => setTab('orders')} />}{tab === 'products' && <Products />}{tab === 'orders' && <Orders />}{tab === 'settings' && <Settings />}</main></div>;
}